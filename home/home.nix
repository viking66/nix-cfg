{ config, pkgs, lib, ... }:
{
 ...
 home.activation = {
   decryptKeys = lib.hm.dag.entryAfter ["writeBoundary"] ''
     $DRY_RUN_CMD ${pkgs.age}/bin/age -d -i ${../key.txt} ${../keys/id_ed25519.age} > $VERBOSE_ARG ~/.ssh/id_ed25519
     $DRY_RUN_CMD chmod 600 $VERBOSE_ARG ~/.ssh/id_ed25519
     
     $DRY_RUN_CMD ${pkgs.age}/bin/age -d -i ${../key.txt} ${../keys/id_rsa.age} > $VERBOSE_ARG ~/.ssh/id_rsa
     $DRY_RUN_CMD chmod 600 $VERBOSE_ARG ~/.ssh/id_rsa
     
     $DRY_RUN_CMD mkdir -p $VERBOSE_ARG ~/.config/sops/age
     $DRY_RUN_CMD ${pkgs.age}/bin/age -d -i ${../key.txt} ${../keys/sops.key.txt.age} > $VERBOSE_ARG ~/.config/sops/age/key.txt
     $DRY_RUN_CMD chmod 600 $VERBOSE_ARG ~/.config/sops/age/key.txt
   '';
 };
